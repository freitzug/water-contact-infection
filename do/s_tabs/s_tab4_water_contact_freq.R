...
noms0 <- c(occ,dom,rec)
dft0 <- df %>% dplyr::select(all_of(c(noms0))) #restrict to ppl with water contact
dft0[dft0==0] <- NA

#dplyr::select vars for table 2
noms <- paste0(c(occ,dom,rec),"_trips_to_water")
...
#write output
write.csv(t,"out/s_tabs/tab_4.csv",row.names = F)

t %>% kable('latex', booktabs = T, longtable=T,
             caption = "Water contact frequency per activity (in trips per week)" ) %>%
  column_spec(1, width = "7cm") %>% #adj col width
  column_spec(2:ncol(t), width = "2.25cm") %>%
  ...
  footnote(general="Median and interquartile range (median [IQR]) are shown. Kruskal-Wallis Rank Sum Test were performed to test for group differences.",
           threeparttable = T) %>%
  save_kable(file = "out/s_tabs/tab_4.tex")
